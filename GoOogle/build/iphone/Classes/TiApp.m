...
- (void)boot
{
	DebugLog(@"[INFO] %@/%@ (%s.6eb8990)",TI_APPLICATION_NAME,TI_APPLICATION_VERSION,TI_VERSION_STR);
...
- (NSString*)systemUserAgent
{
    ...
    NSString *kmyFirsClassicAppUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
    return [NSString stringWithFormat:@"%@/%s (%@)",kmyFirsClassicAppUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo];
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a myFirsClassicApp app using the node.js CLI
