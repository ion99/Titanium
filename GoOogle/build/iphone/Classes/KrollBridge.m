...
NSString * myFirsClassicApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation myFirsClassicAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_myfirsclassicapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create myFirsClassicApp global object
	NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];

	// Load the "myFirsClassicApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_myfirsclassicapp = [[myFirsClassicAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];

	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_myfirsclassicapp];

	NSString *_myfirsclassicappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _myfirsclassicappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_myfirsclassicapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:myFirsClassicApp$ModuleRequireFormat, dirname, filename, code];
...
- (TiModule *)loadJavascriptText:(NSString *)data fromFile:(NSString *)filename withContext:(KrollContext *)kroll
{
	...
	if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
		@throw [NSException exceptionWithName:@"org.myfirsclassicapp.kroll"
